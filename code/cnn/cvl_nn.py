...
from cnn.convolution import ConvolutionType, Reversal, cal_cvl_wh
...
class CVLFNN(FNN):
    ...
    # 卷积类型
    _cvl_type = ConvolutionType.Narrow

    # 是否翻转卷积
    _rev = Reversal.NO_REV

    def __init__(self, cvl, activation=None, last_hop_activation=None, loss=None):
        """
        构造函数
        :param cvl: 卷积对象
    ...
    def _init_other_para(self):
        ...
        # 神经网络输入，维度(3维)
        self._sx_dim = self._sx_list[0].shape

        # 图像宽度，高度，深度
        self._width = self._sx_dim[0]
        self._height = self._sx_dim[1]
        self._depth = self._sx_dim[2]

        # 神经网络输出，向量维度
        # self.sy_dim = self.neuron_count_list[self.layer_count - 1]

        # 初始化 self.layer_count
        self._layer_count = len(self._w_shape_layer)
    ...
    def _init_w_b(self):
        ...
        for layer in range(0, self._layer_count):
            ...
            width, height = cal_cvl_wh(w, x, self._s)
    ...
    def _calc_layer(self, x, layer):
        ...
        # 2、计算卷积结果
        y, err = self._cvl.convolution_sum_depth(w, x)
        ...
        for i in range(0, y_width):
            for j in range(0, y_height):
                for k in range(0, y_depth):
                    y[i, j, k] = self._activation.active(y[i, j, k])
    ...
    def __bp(self, nn_y_list, sy):
        ...
        for k in range(0, depth):
            for i in range(0, width):
                for j in range(0, height):
                    ksi_last[i, j, k] = dy[i, j, k] * self._activation.derivative(nn_y_last[i, j, k])
        ...
        for layer in range(self._layer_count - 2, -1, -1):
            ...
            # 当前层的 ksi
            ksi_cur, dy = self._cvl.convolution(w, ksi_next, Reversal.REV, ConvolutionType.Wide)
    ...
    def __modify_wb_by_ksi_list(self, ksi_list, sx, nn_y_list):
        ...
        for layer in range(0, self._layer_count):
            ...
            w_pd, err = self._cvl.convolution(ksi, v)

            # 修正当前层的 w
            self._w_layer[layer] = np.subtract(w, self._rate * w_pd)  # 不知道3维数组是否可以这样相减

            # 损失函数针对当前层的 b 的偏导(partial derivative)，b_pd 等于 ksi
            b_pd = ksi

            # 修正当前层的 b
            self._b_layer[layer] = np.subtract(b, self._rate * b_pd)  # 不知道3维数组是否可以这样相减
